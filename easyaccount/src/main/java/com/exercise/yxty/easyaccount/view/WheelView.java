...
public class WheelView extends View {
     ...
     * Fling结束
     */
    private static final int FLING_OVER = 0x002;
    /**
     * 移动距离
     */
...
//                    noEmpty();
                ...
                msg.arg2 = distence;
                /*
                    原来程序中将actionUp和noEmpty放置在此处，因为这两个函数中会调用onSelectListener的方法
                    而customer实现接口时会写入UI修改的语句，但这两个函数是在非UI线程运行的，会带来错误

                    原程序似乎是通过endSelect和selecting来区分实现，这里可以通过msg传递数据并通过handler
                    一并在endSelect中实现
...
//        for (ItemObject itemObject : itemList) {
//            Log.i("test", itemObject.id + "," + itemObject.isSelected());
//        }
            ...
            public void run() {
                ...
                for (ItemObject item : itemList) {
                    if (item.isSelected()) {
                        if (onSelectListener != null)
                            /*
                                此函数主要实现手动移动位置不准，缓慢调整位置的目的，因为在actionUP函数中
                                已经调用过endSelect方法，所以此处在此调用会造成问题

                                另外，此处的item.id 和item.itemText并未发生变化，所以没有必要再次调用
...
//        Log.i("test", "controlHeight" + controlHeight + "," + "unitHeight" + unitHeight);
        if (onSelectListener == null)
            return;
        for (ItemObject item : itemList) {
//            Log.i("test", "id=" + item.id + "," + "y=" + item.y + "," +
//                    "move=" + item.move + "isSelect=" + item.isSelected());
            if (item.isSelected()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 0:
                    ...
                    break;
                case FLING_OVER:
                    actionUp(msg.arg1 > 0 ? msg.arg2 - 10 : msg.arg2 * (-1) + 10);
                    noEmpty();
                    break;
                default:
                    break;
    ...
    private class ItemObject {
        ...
        public boolean isSelected() {
            //上边缘线在中心范围内
//            if ((y + move) >= controlHeight / 2 - unitHeight / 2 + 1
//                    && (y + move) <= controlHeight / 2 + unitHeight / 2 - 1)
//                return true;
            //上边缘在中心上半部分
            if ((y + move) >= controlHeight / 2 - unitHeight/2 && (y + move) <= controlHeight / 2 ) {
                return true;
            }
            //下边缘线在中心范围内
...
//                return true;
            //下边缘在中心下半部分
            if ((y + move + unitHeight) >= controlHeight / 2 && (y + move + unitHeight) <= controlHeight / 2 + unitHeight / 2) {
                return true;
            }
            if ((y + move) <= controlHeight / 2 - unitHeight / 2 + 1
                    && (y + move + unitHeight) >= controlHeight / 2
